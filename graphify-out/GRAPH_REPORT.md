...
## Corpus Check
- 190 files · ~4,539,161 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2041 nodes · 3424 edges · 129 communities (122 shown, 7 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 13 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `827aa97e`
...
5. `Dynamic Content Routes (`/[...path]`)` - 30 edges
6. `buildRecipeFromModxDoc()` - 26 edges
7. `$lib/components/Nav.svelte` - 25 edges
8. `Search Route (`/keres`)` - 24 edges
9. `buildRecipeFromModxDoc()` - 23 edges
10. `Pharmacy Route (`/patika`)` - 23 edges
...
Cohesion: 0.08
Nodes (50): decodeDocPathId(), isMagazineCandidate(), isModxReferenceType(), isRootReferenceRow(), parseModxReferenceTargetId(), referenceTargetIds(), shouldSyncRow(), classifyPayload() (+42 more)
...
Cohesion: 0.10
Nodes (22): load(), load(), Category, RecipeTeaser, stripRecipeGatedFields(), toTeaser(), buildReceptsarokCategory(), buildReceptsarokTeasers() (+14 more)
...
Cohesion: 0.13
Nodes (28): AUDIT_PATH, auditSkipNoRecipeBody(), buildRecipeModxIdSet(), CATEGORY_REVIEW_PATH, convertSubRecipesToParsedList(), CREATE_REVIEW_PATH, DATA_PATH, deriveSearchTerms() (+20 more)

### Community 15 - "Community 15"
Cohesion: 0.07
Nodes (69): decodeHtmlEntities(), NAMED_HTML_ENTITIES, applyPhotoCredit(), buildRecipeFromModxDoc(), buildRecipesFromModxDoc(), cardImgFromSrc(), cleanServingParenthetical(), countNutritionValues() (+61 more)
...
Cohesion: 0.12
Nodes (27): recipeTeaserFromHit(), IngredientGroup, IngredientItem, isReceptsarokRecipePath(), isRecipeFree(), KeresRecipeTeaser, LegacyHeroImage, normalizeRecipeAssetSrc() (+19 more)
...
Cohesion: 0.12
Nodes (17): [], snapshot, ads, $app/navigation, snapshot, $lib/ads.js, $lib/components/BannerSide.svelte, $lib/components/BannerTop.svelte (+9 more)
...
Cohesion: 0.13
Nodes (21): load(), buildReceptsarokHome(), Banner, consolidateRecipeImg(), fetchKvizFromFirestore(), getCategories(), getKviz(), getRecipesCacheKeyForDev() (+13 more)

### Community 24 - "Community 24"
Cohesion: 0.15
Nodes (19): encodeDocPathId(), emptyContentFolderPaths(), loadProjectionDocs(), pickDocFields(), PROJECTION_FIELDS, __dirname, loadFullProjectionFromFirestore(), loadProjectionDocsForSync() (+11 more)
...
Cohesion: 0.09
Nodes (22): Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`) (+14 more)
...
Cohesion: 0.16
Nodes (11): $app/forms, nav2, snapshot, $lib/components/ExpertSection.svelte, $lib/components/Hero.svelte, $lib/components/NewsletterCTA.svelte, $lib/components/SubscribeCTA.svelte, $lib/components/TopicGrid.svelte (+3 more)

### Community 30 - "Community 30"
Cohesion: 0.07
Nodes (23): applyModxLinkedRecipeFreeFlags(), targetKey(), stringifyRecipesJson(), applyLocal, AUDIT_PATH, clusters, duplicateClusters, entries (+15 more)
...
Cohesion: 0.14
Nodes (25): uploadProjectionSnapshot(), parseReceptsarokRedirectPath(), loadRecipesFromJson(), arraysEqual(), buildDocIndex(), buildRecipeKeyByModxId(), computeRecipeRelatedChanges(), docRelatedKeys() (+17 more)

### Community 33 - "Community 33"
Cohesion: 0.16
Nodes (16): DocLike, EXPERT_TAGS, hasDoctorAuthor(), homeDocs(), isExpertDoc(), NEWS_TAGS, sortListedDocsByIdDesc(), ThinCard (+8 more)
...
Cohesion: 0.13
Nodes (14): Agent reminders, Analytics, Client-Side Logic (`+page.svelte`), Commands, Data Flow Summary, Expert picks (`expertCards`), Home Route (`/`), Key Libraries & Services (+6 more)
...
Cohesion: 0.17
Nodes (12): Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`) (+4 more)
...
Cohesion: 0.19
Nodes (19): linkedModxIdsForRecipe(), applyPhotoCredit(), buildRecipeFromModxDoc(), buildRecipesFromModxDoc(), cardImgFromSrc(), deriveAuthor(), deriveCategoryDecision(), deriveDocCreatedAtIso() (+11 more)
...
Cohesion: 0.07
Nodes (50): Collection slugs (precomputed at sync time), Dynamic Content Routes (`/[...path]`), Individual document routes, Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`) (+42 more)
...
Cohesion: 0.19
Nodes (18): cleanServingParenthetical(), decodeHtmlEntities(), deriveInstructions(), deriveInstructionsHtml(), deriveServings(), htmlToTextLines(), mergeSplitDecimalIngredientLines(), NAMED_HTML_ENTITIES (+10 more)
...
Cohesion: 0.14
Nodes (10): PageProps, miniSearch, PageProps, patikas, query, $lib/components/Nav2.svelte, $lib/components/Search.svelte, svelte/transition (+2 more)
...
Cohesion: 0.09
Nodes (23): Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`) (+15 more)
...
Cohesion: 0.10
Nodes (21): Data Freshness (`getKviz()` in `src/lib/siteConf.ts`), Individual Quiz Route (`/kviz/[...id]`), Layout Server (`+layout.server.ts`), Leaderboard Route (`/kviz/tabella`), Max Score Calculation (`getKviz()` in `src/lib/siteConf.ts`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`) (+13 more)
...
## Knowledge Gaps
- **1015 isolated node(s):** `config`, `id`, `name`, `note`, `tags` (+1010 more)
...
- **Why does `stringifyRecipesJson()` connect `Community 30` to `Community 32`, `Community 130`, `Community 37`, `Community 8`, `Community 44`, `Community 14`, `Community 23`, `Community 27`, `Community 28`?**
