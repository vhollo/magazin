# Graph Report - magazin  (2026-07-12)

## Corpus Check
- 190 files · ~4,538,223 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2012 nodes · 3385 edges · 131 communities (124 shown, 7 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 13 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `fe0f55fa`
...
5. `buildRecipeFromModxDoc()` - 26 edges
6. `$lib/components/Nav.svelte` - 25 edges
7. `Dynamic Content Routes (`/[...path]`)` - 24 edges
8. `buildRecipeFromModxDoc()` - 23 edges
9. `stringifyRecipesJson()` - 22 edges
10. `$lib/modx/collections` - 22 edges
...
Cohesion: 0.13
Nodes (21): isDescriptionAuthorCompatible(), chooseWinner(), compareRecipeCandidates(), pickRedirectTarget(), createReviewPath, patternsPath, readJson(), recipesPath (+13 more)
...
Cohesion: 0.07
Nodes (54): modx_site_content, modx_site_htmlsnippets, modx_site_tmplvar_contentvalues, decodeDocPathId(), isMagazineCandidate(), isModxReferenceType(), isRootReferenceRow(), parseModxReferenceTargetId() (+46 more)
...
Cohesion: 0.18
Nodes (16): containsWordRun(), countNutritionValues(), countNutritionValuesFromTables(), hasNutritionAndIngredients(), hasRealAuthor(), hasRecipeIngredients(), hasRecipeNutrition(), hasVideo() (+8 more)
...
Cohesion: 0.07
Nodes (66): decodeHtmlEntities(), NAMED_HTML_ENTITIES, applyPhotoCredit(), buildRecipeFromModxDoc(), buildRecipesFromModxDoc(), cardImgFromSrc(), cleanServingParenthetical(), countNutritionValues() (+58 more)
...
Cohesion: 0.09
Nodes (34): recipeTeaserFromHit(), Category, IngredientGroup, IngredientItem, isReceptsarokRecipePath(), isRecipeFree(), KeresRecipeTeaser, LegacyHeroImage (+26 more)
...
Cohesion: 0.11
Nodes (22): [], snapshot, ads, nav2, $app/navigation, snapshot, snapshot, $lib/ads.js (+14 more)

### Community 21 - "Community 21"
Cohesion: 0.13
Nodes (27): normalizeArticlePath(), loadRecipesFromJson(), addArticlesInBatches(), articleToSearchDoc(), buildAndUploadSearchIndex(), buildKeresRecipeTeaser(), changedListedPaths(), createMiniSearch() (+19 more)

### Community 22 - "Community 22"
Cohesion: 0.16
Nodes (15): firebase/auth, actions, actions, GET(), RecipePublished, @sveltejs/kit, db, getAdminAuth() (+7 more)
...
Cohesion: 0.19
Nodes (15): encodeDocPathId(), emptyContentFolderPaths(), loadProjectionDocs(), pickDocFields(), PROJECTION_FIELDS, __dirname, loadFullProjectionFromFirestore(), loadProjectionDocsForSync() (+7 more)
...
Cohesion: 0.22
Nodes (11): decodeHtmlEntities(), deriveInstructions(), deriveInstructionsHtml(), deriveSearchTerms(), htmlToTextLines(), idSlugFromRecipeTitle(), isDescriptionAuthorCompatible(), normalizeText() (+3 more)
...
Cohesion: 0.12
Nodes (12): recipeByKey, recipes, RECIPES_PATH, resolvedKeys, review, REVIEW_PATH, apply, fold() (+4 more)

### Community 45 - "Community 45"
Cohesion: 0.12
Nodes (15): Agent reminders, Analytics, Authentication Logic, Authentication Methods, Authentication State, Commands, Data Flow Summary, Key Libraries & Services (+7 more)
...
Cohesion: 0.21
Nodes (24): RecipePublished, Recipe, recipeSlug(), similarRecipesForTitle(), toLayoutRecipe(), getReceptsarokRecipe(), getRecipes(), getChildModxIds() (+16 more)
...
Cohesion: 0.10
Nodes (17): $app/environment, authReady, authUser, AuthUserType, email, hasReceptsarokAccess, uid, apps (+9 more)
...
Cohesion: 0.24
Nodes (14): linkedModxIdsForRecipe(), applyPhotoCredit(), buildRecipeFromModxDoc(), buildRecipesFromModxDoc(), cardImgFromSrc(), deriveAuthor(), deriveCategoryDecision(), deriveDocCreatedAtIso() (+6 more)
...
Cohesion: 0.09
Nodes (38): Collection slugs (precomputed at sync time), Dynamic Content Routes (`/[...path]`), Individual document routes, Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`) (+30 more)

### Community 81 - "Community 81"
Cohesion: 0.13
Nodes (15): Components, Cross-linking with Magazine, Data Source, Dedupe & sync process (recipes), Dynamic matching, Magazine → Receptsarok redirects (storage & processing), No existing match → sync-create (new recipe), Paywall / Freemium Model (+7 more)

### Community 82 - "Community 82"
Cohesion: 0.20
Nodes (20): cleanServingParenthetical(), countNutritionValues(), deriveServings(), emptyNutritionTable(), mergeSplitDecimalIngredientLines(), NAMED_HTML_ENTITIES, normalizeReadableText(), normalizeServingUnit() (+12 more)
...
Cohesion: 0.35
Nodes (8): getMagazineArticle(), getMagazineCollection(), isCollectionSlug(), rankDocByTags(), bestCollectionSlug(), load(), similarCards(), load()
...
Cohesion: 0.22
Nodes (9): Data Freshness (`getKviz()` in `src/lib/siteConf.ts`), Layout Server (`+layout.server.ts`), Max Score Calculation (`getKviz()` in `src/lib/siteConf.ts`), Page Component (`+page.svelte`), Page Load (`+page.ts`), Quiz Form Route (`/kviz/form`), Quiz List Route (`/kviz`), Quiz Routes (`/kviz`) (+1 more)

### Community 93 - "Community 93"
Cohesion: 0.13
Nodes (19): applyModxLinkedRecipeFreeFlags(), targetKey(), stringifyRecipesJson(), desiredByKey, recipes, RECIPES_PATH, review, REVIEW_PATH (+11 more)
...
Cohesion: 0.15
Nodes (15): purgeNetlifyPaths(), appendRedirectsManifest(), loadRedirectsManifest(), mergeRedirectEntries(), registerRedirectEntries(), redirectPathForManifestEntry(), refreshReceptsarokRedirectsFromManifest(), apply (+7 more)

### Community 102 - "Community 102"
Cohesion: 0.12
Nodes (17): Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Page Component (`+page.svelte`) (+9 more)
...
Cohesion: 0.48
Nodes (5): getReceptsarokHome(), getMagazineStats(), getSiteStats(), load(), load()
...
Cohesion: 0.20
Nodes (10): Individual Quiz Route (`/kviz/[...id]`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`) (+2 more)

### Community 128 - "Community 128"
Cohesion: 0.12
Nodes (16): Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`), Layout Server (`+layout.server.ts`) (+8 more)

### Community 130 - "Community 130"
Cohesion: 0.22
Nodes (9): Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`), Page Component (`+page.svelte`) (+1 more)

## Knowledge Gaps
- **989 isolated node(s):** `config`, `id`, `name`, `note`, `tags` (+984 more)
...
- **Why does `stringifyRecipesJson()` connect `Community 93` to `Community 32`, `Community 37`, `Community 101`, `Community 8`, `Community 44`, `Community 14`, `Community 21`, `Community 23`, `Community 27`, `Community 28`, `Community 30`?**
