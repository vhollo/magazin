...
<script lang="ts">
  ...
  let carous: HTMLDivElement | undefined
  let scroll = false
  let main: number
  let height: number
  ...
  afterUpdate(() => {
    if (!carous) return
    scroll = carous.scrollWidth > main
  })
  const _left = () => {
    if (!carous) return
  ...
  <section bind:clientWidth={main} bind:clientHeight={height} class="relative">
    <div class="carousel carousel-center bg--neutral space-x-4 px-4 py-8 items-stretch w-full" bind:this={carous}>
      <aside class="carousel-item card rounded-sm card--sm bg-base-300 dark:bg-base-300 w-full sm:w-5/12 lg:w-2/5 xl:w-[27.5%] max-h-1/2 pb-2">
