...
<nav class="order-0 md:sticky top-0 z-40 w--full md:navbar max-md:flex-col justify--center !py-0 bg-neutral">
  ...
  <div class="max-md:sticky top-0">
    <a class="mr-auto block p-2" href="/" on:click={_close_nav}><img class="h-12" src={'/assets/logo-diabetes2.svg'} alt="diabetes.hu"></a>
  </div>
  <label for="mobile-nav" aria-label="open sidebar" class="absolute top-2 right-2 bg-neutral z-100 btn btn-lg btn-square btn-ghost md:hidden text-neutral-content">
    <svg
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
      stroke="currentColor"
  ...
  <ul class="mr-auto md:ml-auto md:mr-0 max-md:w-min">
    {#each Object.keys(nav1) as cat}
      <!-- svelte-ignore a11y-no-noninteractive-tabindex -->
      <li tabindex="0" class="drop-col dropdown-hover dropdown-end grow text-nowrap"><!-- md:inline-block  on:blur={_uncheck} -->
        {#if typeof nav1[cat] === 'string'}
          <a href="{nav1[cat]}" class="max-md:p-4 md:p-2 rounded-sm hover:bg-neutral-focus md:menu-title !text-neutral-content text-nowrap font-medium" on:click={_close_nav}>{cat}</a>
        {:else}
          ...
          <ul tabindex="0" class="menu flex-nowrap max-md:collapse-content dropdown-content md:rounded-md bg-neutral text-neutral-content p-0 md:p-2">
...
<nav class="order-0 md:sticky top-0 z-50 w-full md:navbar max-md:flex-col justify--center py-0 bg--neutral md:text-neutral-content md:bg-base-100 md:order-2"> -->
  <!-- <input id="mobile--nav" type="checkbox" bind:checked={_open_nav}/> -->
  <ul class="max-md:w-min md:sticky top-0 z-50 w-full max-md:flex-col py-0">
    {#each Object.keys(nav2) as cat}
      <!-- svelte-ignore a11y-no-noninteractive-tabindex -->
      <li tabindex="0" class="drop-col dropdown-hover last:dropdown-end grow text-nowrap"><!-- md:inline-block  on:blur={_uncheck} -->
        <input type="radio" name="collapse" class="md:hidden" on:change={_scrollIntoView}/>
        <div tabindex="0" role="button" class="max-md:collapse-title md:menu-title !text-neutral-content text-nowrap font-medium cursor-default">{cat}</div>
        <ul tabindex="0" class="menu flex-nowrap max-md:collapse-content dropdown-content md:rounded-md md:bg-base-100 text-neutral-content p-0 md:p-2">
...
<style>
  ...
  @media (width < 48rem) {
    ...
    nav:has(#mobile-nav:checked), nav:has(#mobile-nav:checked) + nav {
      @apply static;
      /* min-height: 100vh; */
    }
    nav > ul {
    ...
    nav:has(#mobile-nav:checked) > ul, nav:has(#mobile-nav:checked) + nav > ul {
      height: auto;
